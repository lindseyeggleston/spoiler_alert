...
BATCH_SIZE = 200
N_CHARS = 98  # number of characters; includes letters, numbers, and punctuation
...
pkeep = tf.placeholder(tf.float32, name='pkeep')  # percent keep/dropout parameter
...
def train_neural_network(X, n_epochs=N_EPOCHS):
    ...
    cost = tf.nn.softmax_cross_entropy_with_logits(logits=Y_logits, labels=Y_flat_)
